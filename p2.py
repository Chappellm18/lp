...
def printstmt():
    advance()
    consume(LEFTPAREN)
    if token.catergory != RIGHTPAREN:
        relexpr()
        while token.catergory == COMMA:
            advance()
            if token.catergory == RIGHTPAREN:
                break
            relexpr()
...
def relexpr():
   expr()
   if token.catergory in [LESSTHAN,LESSEQUAL,EQUAL,NOTEQUAL,GREATERTHAN,GREATEREQUAL] :
       advance()
       expr()
      
def expr():
   term()
   while token.catergory == PLUS or token.catergory == MINUS:
       advance()
       term()
         
def term():
    factor()
    if token.catergory == TIMES:
        advance()
        factor()
    if token.catergory == DIV:
        advance()
        factor()

def factor():
    advance()
    if token.catergory == UNSIGNEDFLOAT:
        advance()
    elif token.catergory == STRING:
        advance()
    elif token.catergory == TRUE:
        advance()
    elif token.catergory == FALSE:
        advance()
    elif token.catergory == NONE:
        advance()
    elif token.catergory == RIGHTPAREN:
        relexpr()
        consume(LEFTPAREN)
    elif token.catergory == PLUS:
        factor()
    elif token.catergory == MINUS:
        factor()
    else:
